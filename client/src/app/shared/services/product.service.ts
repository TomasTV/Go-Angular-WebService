...
import { IReportFilter } from '../models/report-filter.model';
...
export class ProductService {
  ...
  deleteProduct(product: IProduct | undefined) {
    return this.http.delete<any>(
      this.API_URL + this.productUri + '/' + product?.productId,
      {
        reportProgress: true,
        observe: 'events',
      }
  ...
  retrieveProductReport(productFilter: IReportFilter) {
    const headers = new HttpHeaders();
    return this.http
      .post<any>(this.API_URL + this.productUri + '/reports', productFilter, {
        headers,
